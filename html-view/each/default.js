...
module.exports = item => {
  ...
  if (['b', 'big', 'i', 'small', 'tt', 'abbr', 'acronym', 'cite', 'code', 'dfn', 'em', 'kbd', 'strong', 'samp', 'time', 'var', 'a', 'bdo', 'map', 'object', 'q', 'script', 'span', 'sub', 'sup', 'button', 'input', 'label', 'select', 'textarea'].indexOf(item.tagName) !== -1) {
   // if(item.children.length==1&&item.children[0].tagName=="img"){
     let children = item.children
    if((children&&children.find(i=>i.tagName=="img"))){
      item.wxTag = 'view'
    }else{
      item.wxTag = 'text'
    }
  ...
  if (item.attributes && item.attributes.style) {
    item.attributes.styleString = Object.keys(item.attributes.style).map(key =>{
        let cssKey = key.replace(/([A-Z])/g,"-$1").toLowerCase();
        return cssKey + ': ' + item.attributes.style[key]
    } ).join(';')
  }
}
